...
static void queue_data_dtor(struct io_context *ioc)
{
 	if (!hlist_empty(&ioc->cic_list)) {
		...
		cic = hlist_entry(ioc->cic_list.first, struct dev_io_context, cic_list);
		cic->dtor(ioc);
	}
}
...
#if defined(CONFIG_IOSCHED_CFQ) || defined (CONFIG_IOSCHED_FIOPS) || defined(CONFIG_IOSCHED_CFQ_MODULE) || defined(CONFIG_IOSCHED_FIOPS_MODULE)
